...
def get_clubs(tier: int | None = None, db: Session = Depends(get_db)):
    ...
    return {
        tier: [club.model_dump() for club in clubs] for tier, clubs in by_tier.items()
    }
...
def create_match(match: schemas.MatchCreate, db: Session = Depends(get_db)):
    ...
    for mp in match.players:
        db_mp = models.MatchPlayer(
            match_id=db_match.id, player_id=mp.player_id, team=mp.team
        )
        db.add(db_mp)
    db.commit()
    db.refresh(db_match)
...
def list_matches(
    db: Session = Depends(get_db), user: int | None = None, club: int | None = None
):
    query = db.query(models.Match)
    if club:
        query = query.filter(
            (models.Match.team_a_club_id == club)
            | (models.Match.team_b_club_id == club)
        )
...
def individual_leaderboard(db: Session = Depends(get_db)):
    ...
    for p in players:
        ...
        entries.append(
            schemas.LeaderboardEntry(name=p.username, win_rate=win_rate, **stats)
        )
